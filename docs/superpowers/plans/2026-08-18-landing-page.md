...
**Files:** `web/` (new Next.js app), `web/tailwind.config.ts`, `web/next.config.ts`

- [ ] Step 1: `npx create-next-app@latest web --typescript --tailwind --app --eslint --src-dir=false --import-alias "@/*" --use-npm --no-turbopack` inside `.worktrees/frontend/`
- [ ] Step 2: Add brand tokens to `web/tailwind.config.ts` theme.extend.colors: `terracotta #C1592E`,
      `terracottaDark #92401F`, `cream #FAF5EE`, `bgAlt #F1E6D6`, `ink #2B211B`, `inkSoft #6B5D4F`,
      `inkFaint #9A8B7C`, `line rgba(43,33,27,0.14)`
- [ ] Step 3: `npm run build` passes on the untouched scaffold
...
- [ ] Step 1: `npm install next-intl` in `web/`
- [ ] Step 2: Define routing config (locales `['fr','en','pt','es']`, default `'en'`) in `web/i18n/routing.ts`
- [ ] Step 3: `web/middleware.ts` using next-intl's middleware, matcher excluding `_next`/static files —
      root `/` resolves via `Accept-Language` negotiation, falls back to `en`
- [ ] Step 4: Write `web/messages/fr.json`, `en.json`, `pt.json`, `es.json` — copy every string verbatim
...
- [ ] Step 5: `web/i18n/request.ts` wiring `getRequestConfig` to the routing config
...
- [ ] Step 1: `cp` the hero photo from
      `.../scratchpad/memoria-landing/hero-illustration.jpg` to `web/public/images/hero.jpg`
- [ ] Step 2: `cp` the app icon from `.../scratchpad/icon/icon-final-1024.png` to
...
- [ ] Step 1: Move `app/page.tsx`/`app/layout.tsx` scaffold into `app/[locale]/` per next-intl's
      App Router convention
- [ ] Step 2: `app/[locale]/layout.tsx`: `next/font/google` for Playfair Display (weights 400/700/900)
...
- [ ] Step 3: `app/not-found.tsx` — plain not-found page for unknown locale segments
- [ ] Step 4: `npm run build` passes, all 4 locale pages listed in the build output

## Task 5: Header component

**Files:** `web/components/Header.tsx`

- [ ] Step 1: Wordmark "Memória Carioca" (Playfair, bold, small, uppercase tracked)
- [ ] Step 2: Language switcher — real `<Link href="/en">` etc. (next-intl `Link`), active locale
      styled filled-pill terracotta, others outlined
- [ ] Step 3: CTA button "Télécharger l'app" / per-locale copy — `<span>` or disabled-looking element,
...
- [ ] Step 1: Full-bleed section, terracotta gradient background (`#7C3B20 → #B9552B → #D2793F`,
...
- [ ] Step 2: Eyebrow (small dot + uppercase label), `<h1>` "Memória Carioca" (Playfair Black, large),
...
- [ ] Step 3: Responsive check at phone width — text column stays narrow enough not to run under the
...
- [ ] Step 1: `Features.tsx` — 3-column grid (1 column on mobile), each panel: Playfair roman numeral
      (I/II/III) in terracotta, `<h3>` heading, body paragraph, hairline divider between columns
      (not on mobile)
- [ ] Step 2: `Footer.tsx` — wordmark + tagline left, 4 language pills right (same Link pattern as
      Header), hairline-topped copyright row: "© 2026 Memória Carioca" + locale-specific
      footerBottomRight string
- [ ] Step 3: Assemble `app/[locale]/page.tsx` = Header + Hero + Features + Footer
- [ ] Step 4: `npm run build` passes; manually diff against the Claude Design prototype screenshot
...
- [ ] Step 1: On PR touching `web/**`: `npm ci`, `tsc --noEmit`, `next lint`, `next build`, working
      directory `web/`
- [ ] Step 2: Commit
