...
methodology (DATA_PART1-5). The old hardcoded 6-place "SAMPLE" list (pre-grounding-methodology
...
all_entries = DATA_PART1 + DATA_PART2 + DATA_PART3 + DATA_PART4 + DATA_PART5

# Dedupe by name, first occurrence wins (part1 > part2 > part3 > part4 > part5). Known real
# duplicate this order resolves: "Polo Praça XV" appears in both part1 (the original recovered
# 33, already judge-verified) and part5 (independently grounded via fallback search) -- keeps
# part1's version, drops part5's redundant one.
seen = set()
deduped = []
duplicates_dropped = []
